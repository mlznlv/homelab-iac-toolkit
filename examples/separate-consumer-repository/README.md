...
**There is no backend.** `tofu/versions.tf` declares none, because this example creates nothing and has no state worth protecting. That is not a recommendation to use local state for a real deployment: choose a backend and a state-custody policy before your first real apply.
