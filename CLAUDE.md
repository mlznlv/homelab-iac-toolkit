...
- **Publication-safety controls:** [`.gitignore`](.gitignore), which applies to every contributor, and the optional write-time hook described below. Three checks under [`scripts/`](scripts) keep them honest and are wired into both local validation and continuous integration: one asserts that the ignore rules and the hook agree, one checks tracked content for ignored files and personal paths, and one checks the Python lock against its declaration. None of them writes anything or needs network access.
...
- [`.claude/settings.json`](.claude/settings.json) pre-approves inspection and validation commands and registers the hook below. Its command list is limited to `git` and `gh` inspection, to tools this repository declares in `.tool-versions` or `requirements-dev.txt`, and to the repository's own validation entry points and helper scripts. Every entry is non-mutating, which is why the list stops at `task validate` and its focused checks and does not cover regenerating the dependency lock. It pre-approves nothing a contributor could not run by hand, and it decides no check — the repository's validation configuration and CI do that.
