...
- [`.claude/settings.json`](.claude/settings.json) pre-approves inspection and validation commands and registers the hook below. Its command list is limited to `git` and `gh` inspection, to tools this repository declares in `.tool-versions` or `requirements-dev.txt`, and to the repository's own validation entry points and helper scripts. The entries name inspection and validation commands, but a prefix rule cannot exclude a flag: `markdownlint-cli2` and `zizmor` both have a fix mode, and `git branch` can delete a branch, so those entries would admit a mutating invocation as well as the intended one. That imprecision is recorded rather than hidden. Pre-approving a command removes a prompt for something the contributor could already run: it grants no capability and suppresses no check, and whatever it changes still has to pass the checks in this repository and review before it can land. The `.claude/` directory is defense in depth, as the repository design describes it, rather than the layer this repository relies on: `.gitignore`, the validation configuration and CI are the controls that apply to every contributor whatever tools they use.
