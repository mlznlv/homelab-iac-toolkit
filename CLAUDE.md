...
[`.claude/settings.json`](.claude/settings.json) pre-approves inspection and validation commands and registers [`hooks/block-unsafe-writes.sh`](.claude/hooks/block-unsafe-writes.sh), which refuses to write files that must never exist here.
