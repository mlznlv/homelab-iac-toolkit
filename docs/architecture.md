...
Toolkit-owned live testing is deferred. Any future live test must be isolated from normal public CI, explicitly opt-in, and Architecture-reviewed with credential custody, trusted triggers, infrastructure ownership, cleanup, failure handling, and cost responsibility defined before implementation. Introducing one requires an accepted ADR: any toolkit-owned live test puts credential custody into a CI path and produces evidence this repository's compatibility contract currently disclaims, so it changes durable security and validation boundaries by construction. Adjusting an already-accepted live-test design within those boundaries — its runner, its schedule, the mechanics of its trigger — does not.
